...
class Model(QObject):
    ...
    def compute_local_hrv(self):
        ...
        current_ibi_phase = np.sign(
            self._ibis_buffer[-1] - self._ibis_buffer[-2]
    ...
    def compute_biofeedback(self, x):
        ...
        K = (
            self.hrv_target * 0.5
        )  # divide by half to make K the value at which maximum reward is obtained
    ...
    def hrv_buffer(self, value):
        if self._hrv_buffer[0] != -1:  # wait until buffer is full
            ...
            if (
                value > threshold
            ):  # correct hrv values that considerably exceed threshold
        ...
        average_idcs = find_indices_to_average(
            self._ibis_seconds[-HRV_BUFFER_SIZE:], self._hrv_mean_window
        )
